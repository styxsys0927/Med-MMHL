...
import MyFunc
from fake_article_dataset import PD_Dataset, MM_Dataset

from eval_clip import eval
from baseline_models import clip_classifier
...
parser.add_argument('-dataset-type', type=str, default='factcheck_image', help='choose dataset to run [options: article, sentence, tweet, image]') # Should I merge them all?
...
parser.add_argument('-clip-type', type=str, default="openai/clip-vit-base-patch32", help='the pretrained image embedding choice') # to be replaced by https://huggingface.co/models?sort=downloads&search=BERTweet
...
parser.add_argument('-model-name', type=str, default='clip_classifier', help='different structures of metric model, see document for details')
...
tr_dataset = MM_Dataset(args, tr)
dev_dataset = MM_Dataset(args, dev)
te_dataset = MM_Dataset(args, te)
...
# model
if args.model_name == 'clip_classifier': # used for any transformer model
    fk_det_model = clip_classifier.CLIPClassifier(args)
else:
...
if args.test:
    ...
    try:
        ...
        train_clip.train(tr_dataloader, dev_dataloader, fk_det_model, args)
